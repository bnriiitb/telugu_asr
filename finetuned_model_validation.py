...
processor = AutoProcessor.from_pretrained("bnriiitb/whisper-small-te-146h")
model = AutoModelForSpeechSeq2Seq.from_pretrained("bnriiitb/whisper-small-te-146h")
...
vakyansh_texts = []
for item in tqdm(fleurs_dataset['audio'][:2]):
...
vakyansh_wer = 100 * metric.compute(predictions=vakyansh_texts, references=fleurs_dataset['text'][:2])
print(vakyansh_wer)